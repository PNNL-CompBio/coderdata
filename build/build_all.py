...
Upload the latest data to Figshare (ensure tokens are set in the local environment):
  python build/build_all.py --figshare --version 0.1.30
    ...
    parser.add_argument('--dataset',dest='datasets',default='broad_sanger,hcmi,beataml,pancpdo,bladderpdo,sarcpdo,liverpdo,novartispdx,mpnst',help='Datasets to process. Defaults to all available.')
    ...
    if any(dataset in args.datasets for dataset in ['beataml', 'mpnst', 'bladderpdo', 'pancpdo','sarcpdo','liverpdo','novartispdx']) and not synapse_auth_token:
        if args.docker or args.samples or args.omics or args.drugs or args.exp or args.all: # Token only required if building data, not upload or validate.
            raise ValueError("SYNAPSE_AUTH_TOKEN is required for accessing MPNST, beatAML, bladderpdo, pancpdo, liverpdo, novartispdx, or sarcpdo datasets.")
    ...
    # if args.figshare or args.validate:
        ...
        prefixes = ['beataml', 'hcmi', 'cptac', 'pancpdo', 'bladderpdo','sarcpdo', 'genes', 'drugs', 'liverpdo','novartispdx', 'mpnst']

...
def main():
    ...
    To Run:
    --------
    python createPancreaticSamplesFile.py

    Output:
    -------
    A local CSV file named '/tmp/pancreatic_samples.csv' containing the processed metadata.
    """
    parser = argparse.ArgumentParser()
    parser.add_argument('--prevSamples',dest='prev_samps', nargs='?',type=str, default='', const='', help='Previous sample file')
    parser.add_argument('--mapfile',dest='map',help='Mapping to common_cancer from primary_diagnosis and tissue_or_organ_of_origin',default='pancreatic_cancer_types.csv')

    args = parser.parse_args()
    manifest_path = "full_manifest.txt"
    #manifest_url = "https://raw.githubusercontent.com/PNNL-CompBio/candleDataProcessing/hcmi_update/pancreatic/full_manifest.txt"
    ...
    if args.prev_samps is None or args.prev_samps=='':
        print("No Previous Samples file was found. pancreatic Data will not align with other datasets. Use ONLY for testing purposes.")
        maxval = 0
    else:

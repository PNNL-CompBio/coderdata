...
if __name__ == "__main__":
    ...
    sarcoma_samples = pd.read_csv(args.samplesFile)

    sarcoma_drugs = pd.read_csv(args.drugFile, sep="\t")
    ...
    sarcoma_samples['Sample_ID'] = sarcoma_samples['other_id'].str.lower()
    sarcoma_samples["Sample_ID"] = sarcoma_samples["Sample_ID"].str.replace("_1", "-1")
    sarcoma_samples['Sample_ID'] = sarcoma_samples["Sample_ID"].str.split("_", expand =True)[0]
    # and change dashes back to underscores to merge with drug_data's Sample_ID
    sarcoma_samples["Sample_ID"] = sarcoma_samples["Sample_ID"].str.replace("-", "_")
    ...
    experiments = drug_data.merge(sarcoma_drugs, how='left').merge(sarcoma_samples, how='inner')
    ...
    # write to tsv
    toReturn.to_csv('/tmp/sarcoma_experiments.tsv', sep='\t', index=False)

...
More information about the following functions can be found in the [API reference](APIreference.rst).
...
##### Loading data into a `Dataset` object <!-- {#loading-data-into-a-dataset-object} -->
...
-   `Dataset.format(data_type='copy_number', copy_call=False)` returns a `matrix` like `pandas.DataFrame` where cells report the `mean` copy number value for each combination of gene (row - `entrez_id`) and cancer sample (column - `improve_sample_id`).
...
##### Saving manipulated `Dataset` objects (e.g. saving splits) <!-- {#saving-manipulated-dataset-objects} -->

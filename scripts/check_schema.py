...
def _validate_one(task):
    ...
    # A declared file that was never produced is reported as a clean failure
    # rather than crashing the whole validation run with a FileNotFoundError.
    if not os.path.exists(file_path):
        return (
            dataset_name, target_class, file_path,
            [f"[ERROR] declared file not found (was it produced by the build?): {file_path}"],
        )
